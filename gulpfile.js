...
gulp.task('compile_scss', function() {
    ...
    const react_img = gulp.src('node_modules/react-image-gallery/styles/scss/image-gallery.scss').pipe(sass().on('error', sass.logError));
    const scss = gulp.src(SCSS_SRC)
        .pipe(sass().on('error', sass.logError));
    return merge(reset, react_img, scss)
        .pipe(minifyCSS())
